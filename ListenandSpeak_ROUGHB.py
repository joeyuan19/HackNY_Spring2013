...
def main():
  ...
  passes     = 21		# Listen/Hear 10  times then quit
  ...
  while(count<passes):
    ...
    if phase == 0:
      ...
      #count = count + 1
      time.sleep(4)
      Durr = 50 			# Set Duration To 1000 ms == 1 second
  ...
  print "\n\n\t\t", len(send)
  print "\n\n\t\t", len(recieve)
  fileName = "dump/A_SEND.txt"
  DataOut = np.column_stack((send))
  np.savetxt(fileName, DataOut, fmt=('%.6f'))
  fileName = "dump/A_RECIEVE.txt"
  DataOut = np.column_stack((recieve))
  np.savetxt(fileName, DataOut, fmt=('%.6f'))
